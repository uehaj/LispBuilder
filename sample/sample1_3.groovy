public class sample1_3 extends GroovyTestCase {
  ...
  void testBooleanConstantsAndOperators() {
    ...
    assert bx.build{[progn, true]}.eval() == true
    assert bx.build{[progn, false]}.eval() == false
  ...
  void testCompareOperatorAndNegate() {
    ...
    assert bx.build{[eq, 1, 1]}.eval() == true, "hoge[1]"
    assert bx.build{[eq, 1, 2]}.eval() == false, "hoge[2]"
    assert bx.build{[eq, 1.0, 1.0]}.eval() == true
    assert bx.build{[eq, "abc", "abc"]}.eval() == true, "hoge[3]"
    assert bx.build{[eq, "abc", "abx"]}.eval() == false, "hoge[4]"
    assert bx.build{[not, [eq, 1, 1]]}.eval() == false, "hoge[5]"
    assert bx.build{[not, [eq, 1, 2]]}.eval() == true, "hoge[6]"
    assert bx.build{[neq, 1, 2]}.eval() == true, "hoge[7]"
    assert bx.build{[neq, 1, 1]}.eval() == false, "hoge[8]"
